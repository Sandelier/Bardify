...
void I_FinishUpdate(void) {
    ...
    const int x1 = 16, y1 = 12, x2 = 303, y2 = 156;
    const int crop_width = x2 - x1;
    const int crop_height = y2 - y1;

    for (int y = 0; y < crop_height; y++) {
        int src_y = y1 + y;
        int offset = src_y * SCREENWIDTH + x1;
        fwrite(&I_VideoBuffer[offset], 1, crop_width, stdout);
    }
    fflush(stdout);
    ...
    SDL_LockTexture(texture, &blit_rect, &argbbuffer->pixels, &argbbuffer->pitch);
